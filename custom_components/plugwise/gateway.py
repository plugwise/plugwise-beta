...
async def async_setup_entry_gw(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def delete_notification(self):
        ...
        LOGGER.debug(
            "Service delete PW Notification called for %s", api.smile_name
        )  # pragma: no cover
        try:  # pragma: no cover
            deleted = await api.delete_notification()  # pragma: no cover
            LOGGER.debug("PW Notification deleted: %s", deleted)  # pragma: no cover
        except PlugwiseException:  # pragma: no cover
            LOGGER.debug(
